...
|     Item | Value                              |
|---------:|:-----------------------------------|
| Filename | EPAir_03_PM25_NC1819_Processed.csv |
|     Date | 2018-2019                          |
...
colnames(summary_PM25) <- c("County", '$\\mu g/m^3$')#changed column names
summary_PM25[,2] <- round(summary_PM25[,2], 2)# round second column values to two decimal places
knitr::kable(summary_PM25, caption= "Mean Particulates (2.5mm)") #make table. 

colnames(summary_Ozone) <- c("County", '$\\mu g/m^3$')#changed column names
summary_Ozone[,2] <- round(summary_Ozone[,2], 2)# round second column values to two decimal places
...
```{r echo=FALSE, fig.align='left', fig.height=4, fig.width=6, fig.cap= '2018 Monthly Ozone Levels', warning=FALSE}
Ozone_boxplot_18 <- 
  epa_data %>%
  filter(Year==2018) %>%
...
Ozone_boxplot_18
```
```{r echo=FALSE, fig.align='left', fig.height=4, fig.width=6, fig.cap='2019 Monthly Ozone Levels', warning=FALSE}
Ozone_boxplot_19 <- 
  epa_data %>%
  filter(Year==2019) %>%
  mutate(Month=factor(Month)) %>%
...
## Git Repository
(https://github.com/srheschong/EDE_Fall2024.git)
