...
`<`values that are less than specific number
`<=`values that are less than or equal to a specific number
`>`values that are greater than a specific number
`>=`values that are greater than or equal to a specific number
`&`use this when you have multiple conditions to check if you want them both to be true
...
# Choose multiple conditions to filter (character variables)
...
#!= filters out these lakes from the data so they don't show up
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#same as line 96 but cleaner
...
NTL.phys.data.TuesdayLake.1990thru1999 <- filter(TuesdayLake, year4 > 1989 & year4 < 2000)
...
select(NTL.phys.data.temperature.descending, lakename, sampledate, depth, temperature_C)
...
#first, last functions are from dplyr. much better way of doing this than line 169
```
