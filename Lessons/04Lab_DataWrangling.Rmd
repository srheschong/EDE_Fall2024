...
NTL.phys.data.1999 <- filter(NTL.phys.data, year4>1998, year4<2000)
# what code do you need to use, based on the class of the variable? need to use <> since it's an integer

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesday.90tru99 <- 
...
> Answer: dont have control over row numbers. may change without us knowing.
...
NTL.phys.data.TL.90thru99.July <- 
  NTL.phys.data.Tuesday.90tru99 %>% 
...
NTL.phys.data.TL.90thru99.July.surfacewatertemp <- 
  NTL.phys.data.TL.90thru99.July %>% 
  filter(depth==0) %>% 
  summarize(temperature_C, depth) %>% 
  summary(temperature_C)
#the mean surface water temperature is 22.46 deg. C
```
