...
NTL.phys.data.PeterPaul <- read.csv(
  file="./Data/Processed/NTL-LTER_Lake_ChemistryPhysics_PeterPaul_Processed.csv"
  )
...
#different way to write line 64
...
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #how many observations for each nutrient?
...
NTL.nutrient.data.PeterPaul.spread2 <- pivot_wider(NTL.nutrient.data.PeterPaul.gathered, names_from = nutrient, values_from = concentration) #opposite function of pivot_longer
...
#to debug pipe: run everything above current code chunk. run everything before pipe. keep going pipe by pipe. 
